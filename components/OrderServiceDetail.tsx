...
const OrderContent: React.FC<{
    ...
    return (
        <div className={`flex flex-col h-full w-full ${isEmbedded ? 'bg-transparent' : 'absolute inset-0 z-20 transition-all duration-300'} ${!isEmbedded && (isExpanded ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}`}>
            ...
            {/* Header Actions */}
            <div className={`flex items-center justify-between p-4 border-b ${isEmbedded ? (isDark ? 'border-white/5 bg-black/40' : 'border-gray-200 bg-white') : 'bg-black/40 backdrop-blur-3xl border-white/5'} shrink-0`}>
                {!isEmbedded && (
                    <button
            ...
            <div className={`flex-1 overflow-y-auto px-6 py-4 space-y-6 scrollbar-guepardo`}>
                ...
                {order.courier && (
                    <div className={`border rounded-3xl p-5 ${isEmbedded ? (isDark ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200') : (isDark ? 'bg-white/5 border-white/10 shadow-2xl' : 'bg-white border-gray-200 shadow-sm')}`}>
                ...
                <div className="space-y-4">
                    <div className={`rounded-3xl p-5 border flex items-center justify-between transition-all ${isDark ? 'bg-white/[0.03] border-white/5 hover:bg-white/5' : 'bg-gray-50 border-gray-200 shadow-sm'}`}>
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isDark ? 'bg-white/5 shadow-inner' : 'bg-white shadow-sm border border-gray-200'}`}>
                                <User size={24} className={isDark ? 'text-white/30' : 'text-gray-400'} />
                            </div>
                            <div>
                ...
                <div className="px-2 pb-6">
                    <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] mb-6 ${isDark ? 'text-white/30' : 'text-gray-500'}`}>
                         Histórico do Fluxo
                    </h3>
                            ...
                            order.events.map((event, index) => (
                                <div key={index} className="relative pl-8">
                                    ...
                                    <div className="flex flex-col">
                                        <span className={`text-[10px] uppercase font-black tracking-widest ${index === 0 ? 'text-orange-500' : (isDark ? 'text-white/30' : 'text-gray-400')}`}>
                                            {new Date(event.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                        </span>
                                        <span className={`text-sm font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>
                                            {event.label}
                                        </span>
                                            <span className={`text-xs mt-1 italic ${isDark ? 'text-white/50' : 'text-gray-500'}`}>
                                                {event.description}
                                            </span>
                                    </div>
                                </div>
            ...
            {/* Footer Actions */}
            <div className={`p-6 border-t flex gap-4 shrink-0 ${isEmbedded ? (isDark ? 'border-white/10 bg-black/60' : 'border-gray-100 bg-white') : (isDark ? 'bg-black/60 border-white/10' : 'bg-white border-gray-200')}`}>
                <button
                    onClick={(e) => { e.stopPropagation(); window.print(); }}
                    className={`flex-1 h-12 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 border ${isEmbedded ? (isDark ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-100') : (isDark ? 'bg-white/5 hover:bg-white/10 text-white border-white/10' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-100')}`}
                >
